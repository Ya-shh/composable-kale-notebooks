...
def main():
    ...
    general_group.add_argument(
        ...
        type=str,
        nargs="+",
        help="Paths to multiple notebooks to compose into a single pipeline. "
        "Execution order is inferred from data dependencies between them.",
    )
    ...
    if args.workflow:
        from kale.processors.workflow import compose_notebooks_as_subpipelines

        # NotebookConfig uses `base_image`, not the `docker_image` override key.
        workflow_base_image = mt_overrides_group_dict.pop("docker_image", "") or ""
        ...
        print(f"Composed workflow (sub-pipelines), inferred order: {' -> '.join(order)}")
        print(f"dsl_script_path: {dsl_script_path}")
        pipeline_package_path = kfputils.compile_pipeline(dsl_script_path, args.pipeline_name)
        ...
        return
    if not args.nb:
        parser.error("Provide either --nb <notebook> or --workflow <nb1> <nb2> ...")
